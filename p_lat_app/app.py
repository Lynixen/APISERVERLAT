...
def models():
    ...
    parts = request.authorization.token.split('|')
    if not (2 <= len(parts) <= 3):
        raise ValueError("Invalid token format, expected two or three parts")

    if len(parts) == 2:
        p_b_cookie, bot_name = parts
        p_lat_cookie = None 
    else:
        p_b_cookie, p_lat_cookie, bot_name = parts

    if (p_b_cookie != config.get("cookie") or 
        p_lat_cookie != config.get("plat", p_lat_cookie) or  
        bot_name != config.get("bot")):

        config["bot"] = bot_name
        config["cookie"] = p_b_cookie

        if p_lat_cookie is not None:
            config["plat"] = p_lat_cookie
